...
// キーチェーンの中身を読むだけ。書き戻しはしない
// （自前で更新すると Claude Code 側のログインを壊しかねない）。
// ⚠️ 返すのは claudeAiOauth の丸ごと。鍵そのものはここから外へ出さない
//    （/api/state に乗せるのは「いつ切れるか」だけ。値は絶対に乗せない）。
    ...
    execFile('/usr/bin/security',
      ...
      (err, stdout) => {
        ...
        try {
          ...
          resolve((o && o.claudeAiOauth) || null);
...
async function keychainToken() {
  const o = await keychainRead();
  const t = o && o.accessToken;
...
// ------------------------------------------- Claude のログインの寿命（期限）
//
// なぜ要るか（2026-09-05 に実際に起きたこと）:
//   朝5:05 に更新用の鍵（refresh token）の期限が切れていた。手持ちの通行証
//   （access token）はまだ生きていたので、昼まで何も起きなかった。12:05 に
//   通行証が切れて更新に行き、断られ、**キーチェーンの鍵が2つとも空になった**。
//   tmux の席はどれも同じ鍵を読むので、外出中に全席が一度に「Login expired」。
//   このとき画面に出ていたのは残量バーの「?」だけで、何が起きたのか分からなかった。
//
// 期限そのものは相手方の仕様なので、こちらでは延ばせない。できるのは
// **切れる前に言うこと**と、**切れたら「ログインし直しが要る」と名指しすること**。
//
// ⚠️ ここで外に出すのは「状態」と「いつ切れるか」だけ。鍵の中身は出さない。
...
let authNotified = '';                    // 最後に iPhone へ知らせた状態

function authLoad() {
  try {
    const o = JSON.parse(fs.readFileSync(AUTH_FILE, 'utf8'));
    if (o && typeof o.notified === 'string') authNotified = o.notified;
  } catch (e) { /* 無ければ白紙から */ }
}
function authSave() {
  try {
    const tmp = AUTH_FILE + '.tmp';
    fs.writeFileSync(tmp, JSON.stringify({ notified: authNotified }), { mode: 0o600 });
    fs.renameSync(tmp, AUTH_FILE);
  } catch (e) { /* 残せなくても動きは変わらない */ }
}
authLoad();
...
// 待たせない。古ければ裏で読み直して、いま持っている写しを返す（残量と同じ作り）
function authSnapshot() {
  const now = Date.now();
  const ttl = (authCache.value && authCache.value.state === 'ok') ? AUTH_TTL_OK : AUTH_TTL_BAD;
  if (!authFetching && now - authCache.at > ttl) {
    authFetching = keychainRead()
      .then((o) => {
        ...
        authNotify(v.state, v.deadline);
      })
      .catch(() => {})
      .finally(() => { authFetching = null; });
  }
  return authCache.value;
}

// ⚠️ **画面が開いていなくても見張る。** /api/state からしか呼ばないと、iPhone で
// アプリを開いているあいだしか気づけない＝出先で知らせが届かない（それではこの
// 仕組みの意味が無い。今回いちばん困ったのが「離れているあいだに切れた」だった）。
// 1分ごとに様子を見て、実際にキーチェーンを読むのは上の TTL のぶんだけ
// ＝生きていれば5分に1回、切れていれば45秒に1回。
setInterval(authSnapshot, 60 * 1000);
authSnapshot();   // 起動した直後にも1回（再起動をまたいで黙り込まないように）
...
// ⚠️ パッケージ（中身がフォルダだがひとつの書類）は**中に入らない**。深さを広げた結果、
//    `…タブグループ整理.app.backup/Contents/Resources/applet.icns` が「最近」に出てきた
...
// ⚠️ 実測（2026-09-12 点検）＝フォルダ6段に35件・7段は .app の中身だけ。**余裕は0段**なので、
//    ここで落ちたものは `deep` に数えて画面に「途中まで」と出す（無言で消さない）
const RECENT_DEPTH = 7;
...
async function scanRecent(deadline) {
  ...
  //    1本ハングすると `Promise.all` が解けず、while の頭にある締め切りに一生たどり着かない。
  //    そのまま `recentRunning` が残り続け、**以後すべての「最近」が（引っぱって更新でも）
  //    サーバーを再起動するまで死ぬ**＝直したはずの「出たり出なかったり」より悪い。
  ...
  while (queue.length) {
    ...
    await Promise.all(batch.map(async (job) => {
      let entries;
      try {
        entries = await within(fsp.readdir(job.dir, { withFileTypes: true }), left(), 'iCloud');
      } catch (e) { partial = true; return; }    // 返ってこないフォルダは置いていく
      await Promise.all(entries.map(async (e) => {
        if (e.name.startsWith('.')) return;
        const full = path.join(job.dir, e.name);
        if (e.isDirectory()) {
          if (SKIP_DIR.test(e.name) || PKG_DIR.test(e.name)) return;
          // 深さで落としたものは黙って消さず、partial として画面に出す
          // （前は深さ2で828件が無言で消えていた。同じことを繰り返さない）
          if (job.depth >= RECENT_DEPTH) { deep++; partial = true; return; }
          next.push({ dir: full, depth: job.depth + 1 });
          return;
        }
        try {
          const st = await within(fsp.stat(full), left(), 'iCloud');
          out.push({
  ...
  return { list: out, partial, deep };
}

async function recentFiles(fresh) {
  if (!fresh && recentCache.list && Date.now() - recentCache.at < RECENT_TTL_MS) return recentCache;
  // 走っている1本に相乗りする。⚠️ ただし**締め切りを過ぎたものには乗らない**。
  // 乗り続けると、詰まった1本が全員を道連れにして二度と復帰しない
  if (recentRunning && Date.now() < recentRunning.until) return recentRunning.p;
  let mine = null;
  const free = () => { if (recentRunning === mine) recentRunning = null; };
  const p = (async () => {
    try {
      const r = await scanRecent(Date.now() + RECENT_BUDGET_MS);
      recentCache = { at: Date.now(), list: r.list, partial: r.partial, deep: r.deep };
      return recentCache;
    } finally { free(); }
  })();
  mine = { p, until: Date.now() + RECENT_BUDGET_MS + 1500 };
  recentRunning = mine;
  // ⚠️ 解放を finally だけに任せない。それでも返らない相手が出たときの最後の錠前
  const t = setTimeout(free, RECENT_BUDGET_MS + 2000);
  if (t.unref) t.unref();
  return p;
}
...
// ⚠️ 窓は「10分以内」ではなく**同じ日**にする（2026-09-12 点検）。10分だと、25分おきに
//    書き出したフォルダが別々のかたまりになり、9/6 だけで53行＝150行の35%を食っていた。
//    実測でも「リール動画」が7行に割れていた。日付の見出しがあるので、
//    「その日にそのフォルダから出たもの」で1行にするほうが読む側の頭にも合う
...
// その日かどうかは Mac の時計で決める（画面の日付見出しと同じ切り方）
function dayKey(ms) {
  const d = new Date(ms);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ------------------------------------------------ 出先からログインし直す
    //
    // ログインの寿命は延ばせないので、切れたときに**その場で入り直せる**道を作る。
    //   POST /api/login    … その席に `/login` を打つ
    //   GET  /api/loginurl … 画面に出たサインインのURLを拾って返す
    // サインインそのものは iPhone のブラウザで本人が行う。出てきたコードを
    // 貼り戻すのは今までどおり /api/send（本文はログに残さない）。
    ...
    if (p === '/api/loginurl' && req.method === 'GET') {
      const name = url.searchParams.get('name') || '';
      if (!NAME_RE.test(name)) return json(res, 400, { error: 'bad name' });
      // -J ＝端末の幅で折り返された1本のURLをつなぎ直す。これが無いと
    ...
    if (p === '/api/minaoshi' && req.method === 'POST') {
      ...
      if (listed.ok && listed.sessions.some((s) => s.name === name)) {
        const msg = 'いま始めたところです（' + name + '）';
        return json(res, 409, { error: msg, msg });
      }
      ...
      return json(res, 200, { ok: true, name, msg: 'タスクの見直しを始めました（' + name + '）' });
    }
    ...
    if (p === '/api/files' && req.method === 'GET') {
      ...
      try {
        ...
        // files は古い画面（端末に写しが残っている版）のための置き土産。
        // ⚠️ 畳んだ行からではなく**元の一覧から**作る。entries から作ると、
        //    かたまりに入った数百件が古い画面から丸ごと消える
        const files = r.list.slice(0, 120)
          .map((f) => ({ rel: f.rel, mtime: f.mtime, size: f.size }));
        return json(res, 200, {
          root: PREVIEW_ROOT, mode: 'recent', files, entries,
          partial: r.partial, deep: r.deep || 0, total: r.list.length,
        });
      } catch (e) {
