...
## 5. やり取りの出力の置き場 — **プロジェクト → 成果物 → 版 → 種類**
- 出力（レポート・図・画像・調査結果・単発HTML）は **`~/つみき出力/`** に置く。
  実体は iCloud の `Kodai/00_Tsumiki/11_やりとり出力`。つみきリモートの「⋯ → 制作物を見る」がここを映す。
- 形は **`<プロジェクト>/<成果物>/V<番号>/<種類>/<ファイル>`**（2026-09-15 本人の依頼で決めた）。

      Instagram運用/ストーリー_募集/V2/画像/1_ご質問受付.png
                                  /V2/メモ/投稿メモ.md
                                  /V1/…            ← 前の版は消さずに残す

- **置き場は自分で組み立てず、必ずこの道具に聞く**：

      python3 ~/制作物/tsumiki_out.py --list                                   プロジェクトを見る
      python3 ~/制作物/tsumiki_out.py <プロジェクト> <成果物> <ファイル名>        いまの版に置く
      python3 ~/制作物/tsumiki_out.py <プロジェクト> <成果物> <ファイル名> --new  新しい版を作って置く
      python3 ~/制作物/tsumiki_out.py <プロジェクト> <成果物> --versions         版の一覧（新しい順）

  プロジェクト名も成果物名も、既にあるものに**寄る**（大小・全半角・カタカナ/ひらがな・空白のゆれを吸う）。
  初めてなら**フォルダを自動で作る**（許可を求めない）。
...
- **送った物は `00_Tsumiki/受け取り` にも自動で複製される**（`SendUserFile` の PostToolUse フック → `tsumiki_inbox.py --hook`）。
  iPhone のファイルアプリの「最近使った項目」には、その iPhone で開いた物しか並ばないため（2026-09-23 本人の指摘・本人が「よく使う項目」に登録ずみ）。
  片付けるのは自分で置いた複製だけ（控え＝`~/.cache/tsumiki/inbox.json`・新しい10件）。本人のファイルは消さない・上書きしない。
  置き場の外のファイルを送ると、フックが Claude と本人の両方に知らせる。受け取りが無いときは作り直さず、場所を聞く。
- **版（V）を上げるとき**＝本人に見せたあとで作り直すとき・中身や見た目を大きく変えるとき（`--new`）。
  同じ作業の中でファイルを足す・直すだけなら、いまの版に置く。**前の版は消さない。**
- **種類**は拡張子で道具が決める＝`画像`／`書類`（pdf・html）／`動画・音声`／`メモ`（md・txt）／`データ`（json・csv・台本など）。
  フォルダとファイルを同じ階層に混ぜない（2026-09-06 に iCloud 全体で通した方針）。
- 名前は日本語。**ファイル名に日付や v2 を付けない**（版はフォルダが持ち、日付は更新日時で分かる）。
  プロジェクト名は「お客様名・アプリ名・案件名」、成果物名は「作り直していく1つのもの」（`ご返信カード`・`ストーリー_募集`）。
- ⚠️ **プロジェクト直下にファイルを置かない。** 2026-09-04 に、直下が155個の横一列になって探せなくなり、
  20個のプロジェクトに仕分け直した（元に戻す台本＝`~/.tsumiki-remote/undo-やりとり出力-20260904.sh`）。
- 前の書き方（`<プロジェクト> <ファイル名>`）で呼ぶと、ファイル名から成果物名を作って入れ、書き方を注意する。
