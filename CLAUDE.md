...
- 置き場を聞いた時点で、そのファイルには**自動で印が付く**（`tsumiki_pin.py`）。つみきリモートの席の画面の帯
  「さっき作ったもの」に、探索の取りこぼしと関係なく必ず出る。道具を通さずに作ったものは
  `python3 ~/制作物/tsumiki_pin.py <ファイル>` で後から印を付けられる（置き場の外は受け付けない）。
