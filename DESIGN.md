...
- **Don't** グラデーション・ガラス・発光・大きな影を足す。帳面は光らない。
  **例外**：ぐにゃりタブ（Rare UI「Gooey nav」・`design_refs/rareui.motion.md` ①）の「首」だけは、
  となり合う2つの札の色をつなぐ SVG のグラデーションを使ってよい。切り替えの 0.2秒だけ見える形で、
  面の飾りではないため（2026-09-15 本人が採用。ふたりカードの絞り込み・さきゆきの期間タブ）。
